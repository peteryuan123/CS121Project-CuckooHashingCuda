        ...
        {   
            ...
            for (int data_size = 10; data_size < 25; data_size++) 
            {
                ...
                for (int iter = 0; iter < 5; iter++) // 5 iter experiment
                {
                    ...
                    /* Hash class */
                    cuckoo_serial hash_serial = cuckoo_serial(TABLE_SIZE, num_funcs);
                    cuckoo_cuda hash_cuda = cuckoo_cuda(TABLE_SIZE,num_funcs);
        ...
        for (int num_funcs = 3; num_funcs < 5 ; num_funcs++)
        {
            ...
            for (float scale = 1.1; scale < 2.0; scale+=0.1)
            {
                ...
                for (int iter = 0; iter < 5; iter++)
                {
                    ...
                    cuckoo_serial hash_serial = cuckoo_serial(hash_size,num_funcs);
                    cuckoo_cuda hash_cuda = cuckoo_cuda(hash_size, num_funcs);
